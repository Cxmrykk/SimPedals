/*
 * 3mm Pedal Base Plate (Mirrored)
 * Generated from DXF: "Metal Components/Steel Parts/3mm Pedal Base Plate Mirrored x3.DXF"
 * Modified for native 3D printing by applying 90-degree folds and a precise horizontal 3D mirror.
 */
...
function arc_points(cx, cy, r, a1, a2, cw=false, fn=100) =
    cw ? 
        let(s = a1 < a2 ? a1 + 360 : a1, e = a2, steps = max(2, ceil(abs(s - e) / 360 * fn)))
        [ for (i = [0 : steps]) [ cx + r * cos(s - i * (s - e) / steps), cy + r * sin(s - i * (s - e) / steps) ] ]
    :
        let(s = a1, e = a2 < a1 ? a2 + 360 : a2, steps = max(2, ceil(abs(e - s) / 360 * fn)))
        [ for (i = [0 : steps]) [ cx + r * cos(s + i * (e - s) / steps), cy + r * sin(s + i * (e - s) / steps) ] ];

outer_profile_points = concat(
    [ [3.0, 0.0], [17.0, 0.0] ],
    arc_points(17.0, 3.0, 3.0, 270, 0, false, $fn),
    [ [20.0, 15.0707963268], [19.0, 16.0707963268], [20.0, 17.0707963268], 
      [150.0, 17.0707963268], [151.0, 16.0707963268], [150.0, 15.0707963268] ],
    arc_points(153.0, 3.0, 3.0, 180, 270, false, $fn),
    [ [167.0, 0.0] ],
    arc_points(167.0, 3.0, 3.0, 270, 0, false, $fn),
    [ [170.0, 15.0707963268], [169.0, 16.0707963268], [170.0, 17.0707963268], [170.0, 34.9622488511] ],
    ...
    [ [0.0, 17.0707963268], [1.0, 16.0707963268], [0.0, 15.0707963268], [0.0, 3.0] ],
...
module internal_holes_and_slots() {
    ...
    polygon(points=[ [117.9312213654, 47.1214784482], [127.5160860975, 50.6100839101],
                     [128.6105505561, 47.6030675236], [119.0256858241, 44.1144620617] ]);
    
    polygon(points=[ [126.8921491206, 22.5015317836], [136.4770138526, 25.9901372455],
                     [137.5714783112, 22.9831208590], [127.9866135792, 19.4945153971] ]);
}

module base_plate_2d() {
    difference() { polygon(points=outer_profile_points); internal_holes_and_slots(); }
}
...
module base_plate_3d(thickness = 3.0) {
    ...
    linear_extrude(height = thickness, convexity = 8)
        intersection() { base_plate_2d(); translate([-100, y_fold]) square([400, 400]); }
        
    translate([0, y_fold - thickness, 0])
        rotate([-90, 0, 0])
            translate([0, -y_fold, 0])
                linear_extrude(height = thickness, convexity = 8)
                    intersection() { base_plate_2d(); translate([-100, -200]) square([400, 200 + y_fold]); }
...
// Ensure the part is correctly inverted to serve as the opposite-hand plate
translate([170.0, 0, 0])
    mirror([1, 0, 0])
